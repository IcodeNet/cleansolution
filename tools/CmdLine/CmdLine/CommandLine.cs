...
namespace CmdLine
{
    ...
    public static class CommandLine
    {
        ...
        /// <summary>
        ///   Name of the text reader uses for StdIn
        /// </summary>
        private const string SyncTextReader = "SyncTextReader";

        /// <summary>
        ///   Name of the text writer used for Stdout / Stderr
        /// </summary>
        private const string SyncTextWriter = "SyncTextWriter";
...
(?<Value>.*)\Z";
        ...
        /// <summary>
        ///   indicates that stderr is redirected
        /// </summary>
        private static bool errorRedirected;

        /// <summary>
        ///   indicates that input is redirected
        /// </summary>
        private static bool inputRedirected;

        /// <summary>
        ///   indicates that output is redirected
        /// </summary>
        ...
        /// <summary>
        ///   Gets a value indicating whether ErrorRedirected.
        /// </summary>
        public static bool IsErrorRedirected
        {
            get
            {
                return errorRedirected || IsHandleRedirected(StdHandle.Stderr);
            }
        }

        /// <summary>
        ///   Gets a value indicating whether InputRedirected.
        /// </summary>
        public static bool IsInputRedirected
        {
            get
            {
                return inputRedirected || IsHandleRedirected(StdHandle.Stdin);
            }
        }

        /// <summary>
        ///   Gets a value indicating whether OutputRedirected.
        /// </summary>
        public static bool IsOutputRedirected
        {
            get
            {
                return outputRedirected || IsHandleRedirected(StdHandle.Stdout);
            }
        }
        ...
        private static string TokenizePattern
        {
            get
            {
                return UserTokenizePattern
                       ??
                       string.Format(
                           TokenizeExpressionFormat,
                           GetCaseSensitiveOption(),
                           string.Join(string.Empty, CommandSeparators),
        ...
        ///   The parse.
        /// </summary>
        /// <typeparam name="T"> The type of the command argument class </typeparam>
        /// <returns> A class populated with arguments from the command line </returns>
        /// <exception cref="CommandLineHelpException">The command line could not be parsed</exception>
        public static T Parse<T>() where T : class, new()
        {
            Debug.WriteLine("Parsing argument class [{0}] Command Line: [{1}]", typeof(T), Text);
        ...
        /// </returns>
        public static char Pause(string text = "Press any key to continue...", ConsoleColor color = ConsoleColor.Yellow)
        {
            WriteLineColor(color, text);

            return IsOutputRedirected && !IsInputRedirected
                       ? NullChar
                       : ReadChar();
        }
        ...
        public static char PromptKey(string prompt, params char[] allowedKeys)
        {
            ...
            do
            {
                try
                {
                    ...
                    keyChar = Console.ReadKey(true).KeyChar.ToString(CultureInfo.InvariantCulture).ToLowerInvariant()[0];
                ...
                {
                    Console.WriteLine("Unable to read console input, cancelling clean operation");
                    Environment.Exit(1);
                }
            }
            while (!validKey);
        ...
        /// <summary>
        ///   Reads the next character from the Console InStream
        /// </summary>
        /// <returns> The character read on null if no character was available </returns>
        public static char ReadChar()
        {
            if (IsInputRedirected)
            {
                var charNum = Console.In.Read();
                return charNum != -1 ? (char)charNum : NullChar;
            }
            else
            {
        ...
        ///   The tokenize.
        /// </summary>
        /// <returns> A list of CommandArguments </returns>
                ...
                for (var i = 0; i < matches.Count; i++)
                {
                    for (var j = 0; j < matches[i].Groups.Count; j++)
                    {
                        Debug.WriteLine(
                            "Token[{0}].Groups[{1}] =\"{2}\"",
                            i,
                            RegexTokenize.GroupNameFromNumber(j),
                            matches[i].Groups[j]);
                    }
                }
            }
        ...
        /// The h console handle. 
        /// </param>
        /// <param name="mode">
        /// The lp mode. 
        /// </param>
        /// <returns>
        /// The get console mode. 
        /// </returns>
        ...
        /// <returns> An initialized command line argument </returns>
        ...
        /// <summary>
        /// Determines if a handle is redirected
        /// </summary>
        /// <param name="handleType">
        /// The handle type. 
        /// </param>
        /// <returns>
        /// true if is handle redirected. 
        /// </returns>
        private static bool IsHandleRedirected(StdHandle handleType)
        {
            var handle = GetStdHandle(handleType);

            uint mode;
            return ((GetFileType(handle) & FileType.Char) != FileType.Char) || !GetConsoleMode(handle, out mode);
        }
        ...
        private class CommandLineParametersCollection
        {
            ...
            /// </summary>
            /// <exception cref="CommandLineRequiredArgumentMissingException">A required argument was missing</exception>
            ...
            /// </param>
            /// <returns>
            /// True if the parameter is a parameter. 
            /// </returns>
            ...
            /// There was a problem with the parameter
            /// </exception>
                        ...
                        new CommandArgumentHelp(
                            ...
                            parameter.IsCommand()
                                ...
                                : string.Format(
                                    ...
                                    parameter.Attribute.ParameterIndex,
            ...
            private void Load()
            {
                ...
                if (this.Parameters.Count == 0)
                {
                    ...
                    CommandLineParameterAttribute.ForEach(
                        this.ArgumentType, InferCommandLineParameterAttribute, this.Add);
                }
            }
            ...
            /// </summary>
            /// <exception cref="CommandLineException">The positional arguments were not in order</exception>
