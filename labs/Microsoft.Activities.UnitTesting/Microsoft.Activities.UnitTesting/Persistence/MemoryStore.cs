// --------------------------------------------------------------------------------------------------------------------
// <copyright file="MemoryStore.cs" company="Microsoft">
...
namespace Microsoft.Activities.UnitTesting.Persistence
{
    ...
    /// <summary>
    /// Provides a memory based instance store for testing
    /// </summary>
    public class MemoryStore : InstanceStore
    {
        ...
        /// The known types.
        /// </param>
        public MemoryStore(params Type[] knownTypes)
        {
        ...
        /// The known types.
        /// </param>
        protected MemoryStore(IEnumerable<Type> knownTypes)
        {
            this.serialization = new SerializationProvider(knownTypes);
        }
        ...
        {
            WorkflowTrace.Verbose(">> MemoryStore Command Counts <<");
            WorkflowTrace.Verbose("LoadWorkflowCommand count = " + LoadWorkflowCommandCount);
            WorkflowTrace.Verbose("LoadWorkflowByInstanceKeyCommand count = " + LoadWorkflowByInstanceKeyCommandCount);
            WorkflowTrace.Verbose("SaveWorkflowCommand count = " + SaveWorkflowCommandCount);
            WorkflowTrace.Verbose("CreateWorkflowOwnerCommand count = " + CreateWorkflowOwnerCommandCount);
            WorkflowTrace.Verbose("DeleteWorkflowOwnerCommand count = " + DeleteWorkflowOwnerCommandCount);
        }
        ...
        {
            WorkflowTrace.Verbose(string.Empty);
            WorkflowTrace.Verbose(">> Begin MemoryStore Dump <<");
            WorkflowTrace.Verbose(">> Owners");
            Debug.Indent();
            foreach (var owner in owners)
            {
                WorkflowTrace.Verbose("Owner " + owner.Key);
                Debug.Indent();
                WorkflowTrace.Verbose("LockToken: " + owner.Value.LockToken);
            ...
            WorkflowTrace.Verbose(">> Instances");
            Debug.Indent();
            foreach (var instance in instances)
            {
                WorkflowTrace.Verbose("Instance " + instance.Key);
                Debug.Indent();
                WorkflowTrace.Verbose("Owner:       " + instance.Value.Owner);
                WorkflowTrace.Verbose("LockVersion: " + instance.Value.LockVersion);
                WorkflowTrace.Verbose("Completed:   " + (instance.Value.Completed ? "True" : "False"));
            ...
            WorkflowTrace.Verbose(">> Keys");
            Debug.Indent();
            foreach (var key in keys)
            {
                WorkflowTrace.Verbose("Key " + key.Key);
                Debug.Indent();
                WorkflowTrace.Verbose("Instance:  " + key.Value.Instance);
                WorkflowTrace.Verbose("Completed: " + (key.Value.Completed ? "True" : "False"));
                DumpPropertyBag("Metadata", key.Value.Metadata);
            }

            Debug.Unindent();
            WorkflowTrace.Verbose(">> End MemoryStore Dump <<");
            WorkflowTrace.Verbose(string.Empty);
        }
            ...
            object state)
        {
            ...
            if (save != null)
            {
                WorkflowTrace.Verbose("MemoryStore: SaveWorkflowCommand");
            ...
            if (load != null)
            {
                WorkflowTrace.Verbose("MemoryStore: LoadWorkflowCommand");
            ...
            if (loadByKey != null)
            {
                WorkflowTrace.Verbose("MemoryStore: LoadWorkflowByInstanceKeyCommand");
            ...
            if (createOwner != null)
            {
                WorkflowTrace.Verbose("MemoryStore: CreateWorkflowOwnerCommand");
            ...
            if (deleteOwner != null)
            {
                WorkflowTrace.Verbose("MemoryStore: DeleteWorkflowOwnerCommand");
            ...
            if (tryLoad != null)
            {
                WorkflowTrace.Verbose("MemoryStore: TryLoadRunnableWorkflowCommand");
        ...
        private static void DumpPropertyBag(string bagName, Dictionary<XName, SerializedValue> bag)
        {
            ...
            WorkflowTrace.Verbose(bagName);
            Debug.Indent();
            foreach (var property in bag)
            {
                WorkflowTrace.Verbose("Property: " + property.Key + (property.Value.WriteOnly ? " (WriteOnly)" : string.Empty));
                WorkflowTrace.Verbose(">> Begin Data -------------------------------------------------------");
                if (property.Value.Buffer != null)
                {
                    WorkflowTrace.Verbose(new StreamReader(property.Value.Buffer).ReadToEnd());
                    property.Value.Buffer.Seek(0, SeekOrigin.Begin);
                }

                WorkflowTrace.Verbose(">> End Data-------------------------------------------------------");
            }
        ...
        private struct Instance : ICloneable
        {
            ...
            public object Clone()
            {
                var clone = new Instance
                    { Completed = this.Completed, Owner = this.Owner, LockVersion = this.LockVersion };
                if (this.Data != null)
                {
